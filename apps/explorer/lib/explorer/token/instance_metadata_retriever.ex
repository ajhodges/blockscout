defmodule Explorer.Token.InstanceMetadataRetriever do
  ...
  def fetch_json(%{@token_uri => {:ok, ["data:application/json," <> json]}}, hex_token_id) do
    ...
    e ->
      Logger.debug(["Unknown metadata format #{inspect(json)}. error #{inspect(e)}", __STACKTRACE__],
  ...
  def fetch_json(%{@token_uri => {:ok, ["data:application/json;base64," <> base64_encoded_json]}}, hex_token_id) do
    ...
    e ->
      Logger.debug(
        ["Unknown metadata format base64 #{inspect(base64_encoded_json)}. error #{inspect(e)}", __STACKTRACE__],
        fetcher: :token_instances
      )
  ...
  defp fetch_metadata_inner(uri, hex_token_id) do
    ...
    e ->
      Logger.debug(["Could not send request to token uri #{inspect(uri)}. error #{inspect(e)}", __STACKTRACE__],
        fetcher: :token_instances
      )
